...
def write_baselines(project: ProjectInfo) -> None:
    ...
    for baseline_path, rows, width, identity_width, counts in (
        ...
        previous = {
            row[:identity_width]: row for row in _load_tsv(baseline_path, width)
        }
        for row in rows:
            ...
            if any(int(row[index]) > int(prior[index]) for index in counts):
                regressions.append(f"{baseline_path.name}: {'/'.join(identity)}")
            approved_rows[baseline_path].append(
                (*row[:-1], prior[-1]) if width > 2 else row
            )

    previous_helpers = {
        (directory, function): (files, reason)
        for directory, function, files, reason in _load_tsv(
            HELPER_DUPLICATES_BASELINE, 4
        )
    }
    approved_helpers = []
    for directory, function, files, _reason in helper_rows:
        prior = previous_helpers.get((directory, function))
        if prior is None or not set(files.split(",")).issubset(
            set(prior[0].split(","))
        ):
            regressions.append(
                f"{HELPER_DUPLICATES_BASELINE.name}: {directory}/{function}"
            )
            continue
