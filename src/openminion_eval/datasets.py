...
def build_eval_dataset_template(*, family: str = "generic") -> dict[str, Any]:
    case = _template_case(family)
    return {
        "dataset_version": DATASET_VERSION,
        "name": f"{family}-starter",
        "cases": [case],
        "metadata": {
            "family": family,
            "description": "Starter dataset generated by openminion-eval.",
        },
    }


def write_eval_dataset_template(path: str | Path, *, family: str = "generic") -> Path:
    target = Path(path)
    target.parent.mkdir(parents=True, exist_ok=True)
    target.write_text(
        json.dumps(build_eval_dataset_template(family=family), indent=2, sort_keys=True)
        + "\n",
        encoding="utf-8",
    )
    return target
...
def _template_case(family: str) -> dict[str, Any]:
    prompts = {
        "routing": "Route this coding request to the implementation-capable path.",
        "tools": "Use the appropriate tool only if it is needed.",
        "freshness": "Answer with current information only when the question requires it.",
        "closure": "Complete the user request and state the outcome.",
        "policy": "Refuse unsafe instructions while preserving helpful alternatives.",
        "skills": "Select the named skill and summarize the first concrete step.",
    }
    prompt = prompts.get(family, "Answer this eval prompt with the expected text.")
    return {
        "id": f"{family}-starter-001",
        "name": f"{family}-starter-001",
        "turns": [
            {
                "user": prompt,
