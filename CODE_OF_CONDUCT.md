# Code of Conduct

## Our standard

We are committed to a respectful, constructive, and inclusive community.

Examples of expected behavior:

1. Be respectful and professional.
2. Assume good intent and discuss ideas, not people.
3. Give and receive feedback constructively.
4. Help keep discussions focused and collaborative.

Examples of unacceptable behavior:

1. Harassment, discrimination, or personal attacks.
2. Threatening, abusive, or hateful language.
3. Doxxing or sharing private information without consent.
4. Deliberate disruption, trolling, or bad-faith engagement.

## Scope

This Code of Conduct applies in project spaces (issues, PRs, discussions, chat, and reviews).

## Enforcement

Maintainers are responsible for enforcing this Code of Conduct and may remove, edit, or reject content or contributions that violate it.

Possible actions include:

1. Warning
2. Temporary restriction
3. Permanent ban from project spaces

## Reporting

Report concerns to maintainers through the project’s private contact method or by asking maintainers for a private channel.

## Attribution

This policy is inspired by common open source community standards and adapted for this project.
